...
// to sign in through the physician app. checkAccountStatus now checks
// role server-side right after the caller identifies themselves by email
// (see functions/src/auth.ts's accountRoleInfo and LoginScreen's own
// allowedRoles), so this account is rejected immediately — it never
// reaches set-password, MFA enrollment, or work-location setup at all,
// unlike before this existed (see git history for the version of this
// test that had to walk through all three to prove the same boundary).
//
...
void main() {
  patrolTest(
    'an ems-only account is denied access to the physician app immediately after entering their email',
    ($) async {
      ...
      // no MFA, no work-location setup. checkAccountStatus's own
      // roleAllowed field is what makes this immediate, not
      // AppRouteGuard's post-auth role tier (which still exists as the
      // real enforcement backstop, just isn't what a legitimate account
      // ever has to walk through to be told this).
      await pumpUntil(
        $,
      ...
      expect(find.textContaining("doesn't have access to the AmDash — Physician app"), findsOneWidget);
      expect(
      ...
      expect(find.byType(MainViewScreen), findsNothing, reason: 'must never reach the real physician main view');

      // This account is ems-only, so exactly the EMS app link should be
      // offered — not Physician or Admin.
