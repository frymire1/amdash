...
void main() {
  patrolTest(
    'signs in, adds a patient, edits it, and deletes it',
    ($) async {
      ...
      final patientName = 'Patrol EMS Test Patient ${DateTime.now().millisecondsSinceEpoch}';
      ...
      await pumpUntil($, () => find.text(patientName).evaluate().isNotEmpty, maxIterations: 40);
      expect(find.text(patientName), findsOneWidget, reason: 'patient should appear on the home screen after upload');
      ...
      await tapKey($, 'patient_upload_submit');
      await pumpUntil($, () => find.text('95 bpm').evaluate().isNotEmpty, maxIterations: 40);
      expect(find.text('95 bpm'), findsOneWidget, reason: 'edited heart rate should show on the home screen');
      ...
      expect(find.text(patientName), findsNothing, reason: 'patient should be gone after delete');
    },
  );
}
