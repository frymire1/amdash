// Phase 0 verification: runs against real Chrome (via
...
// real Firebase Auth + Firestore connectivity against amdash-dev before
// any more app code gets written. The throwaway account this signs in
// with is created directly via the Firebase Admin SDK (same pattern used
// by every e2e suite's own support/admin.ts in the Angular apps) and is
// not created by this test — pass its email/password via --dart-define.
import 'package:ems/firebase_options.dart';
import 'package:ems/main.dart';
import 'package:ems/signed_in_screen.dart';
...
void main() {
  ...
  testWidgets('signs in against a real amdash-dev account', (tester) async {
    ...
    expect(find.byType(SignedInScreen), findsOneWidget);
