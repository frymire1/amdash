...
const baseURL = process.env['BASE_URL'] || 'https://amdash-admin-dev.web.app';
...
export default defineConfig({
  testDir: './src',
  outputDir: path.join(workspaceRoot, 'dist/.playwright/apps/admin-e2e/test-output'),
  fullyParallel: true,
  forbidOnly: false,
  retries: 0,
  reporter: [
    [
      'html',
      {
        outputFolder: path.join(workspaceRoot, 'dist/.playwright/apps/admin-e2e/playwright-report'),
        open: 'on-failure',
      },
    ],
  ],
  ...
  // No webServer here — these tests hit the deployed amdash-admin-dev
  // hosting site directly (see baseURL above), not a locally-started dev
  // server. Redeploy before running these if you have unreleased local
  // changes.
  projects: [
    {
      name: 'chromium',
